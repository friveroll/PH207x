...
library(epiR)
@
We can get all the required tables for Incidence rate using a loop (see next question to see a single table and some comments for the code, also you can get \href{here}{} the R script)
